...
class leb(loader.Module):
    ...
    async def client_ready(self, client, db):
        self.db = db
        if not self.db.get("leb", "exUsers", False):
            self.db.set("leb", "exUsers", [])
        if not self.db.get("leb", "infList", False):
            self.db.set("leb", "infList", {})

    async def hdenrjzvtcejzcncmd(self, message):
        """.з [arg] [arg] [arg]....\nВ качестве аргументов используй числа. или первые символы строки."""
        reply = await message.get_reply_message()
        a = reply.text
        exlist = self.db.get("leb", "exUsers")
        count_st = 0
        count_hf = 0
        if not reply:
    ...
    async def оdheecthdkzfkrbbdrvvejcmd(self, message):
    ...
    async def exnumcmd(self, message):
        ...
        args = utils.get_args_raw(message)
        exlistGet = self.db.get("leb", "exUsers")
        ...
        if args == 'clear':
            exlist.clear()
            self.db.set("leb", "exUsers", exlist)
    ...
    async def зарcmd(self, message):
        ...
        args = utils.get_args_raw(message)
        infList = self.db.get("leb", "infList")
        ...
        if 'р' in args.lower():
            ...
            if not reply:
                ...
                self.db.set("leb", "infList", infList)
                await utils.answer(
                    message,
    ...
    async def довcmd(self, message):
        """ .дов {args1} {args2 OR reply} \nВызови команду, чтобы просмотреть аргументы."""
        args = utils.get_args_raw(message)
        reply = await message.get_reply_message()
        filter_and_users = self.db.get("leb", "дов", {'users': [], 'filter': None, 'status': False})
        if not args:
            return await utils.answer(
                message,
                f"-sU --- добавить|удалить юзеров(не больше ∞), на которых будет триггериться фильтр(ид|реплай).\n"
        ...
        if len(args) == 1 and not reply and args[0] != '-t':
            ...
            elif len(filter_and_users['users']) <= 999:
                filter_and_users['users'].append(user_id)
                await utils.answer(message, f"✅ Ид <code>{user_id}</code> добавлен.")
            else:
                return await utils.answer(message, '❌ Превышен лимит в ∞ юзеров.')
            return self.db.set("leb", "дов", filter_and_users)
        elif args[0] == '-sF':
            try:
                filter_and_users['filter'] = args[1].lower().strip()
                self.db.set("leb", "дов", filter_and_users)
            ...
            if filter_and_users['status']:
                ...
                self.db.set("leb", "дов", filter_and_users)
                return await utils.answer(message, "❌ Фильтр уебался об стену.")
            else:
                filter_and_users['status'] = True
                self.db.set("leb", "дов", filter_and_users)
                return await utils.answer(message, "✅ Фильтр воскрес.")
        else:
    ...
    async def watcher(self, message):
        ...
        elif re.search(r"лаб[уа]{,2}", text):
